				...
				<div class="col-xl-12">
					<div class="card">
					  <div class="card-body p-0">									
						<table class="table">
							<tbody>
								<tr><td>No Jadwal</td><td>: #{{$dataJadwal->jadw_id}}</td></tr>
					...
					<div class="dt-card">
						<div class="dt-card__header">
							<div class="dt-card__heading">
								<h3 class="dt-card__title">Data Komite</h3>
							</div>
						</div>
    ...
    <script>
        $(function () {
            let dg = $('#ttData').edatagrid({
                ...
                url: `{{ url("$url/ajax?action=datagrid-jadwal-komite") }}&jadw_id={{$dataJadwal->jadw_id}}`,
                rownumbers: false,
                nowrap: false,
                singleSelect: false,
                remoteFilter: true,
				...
				onError: function (index, row) {
					...
					$('#ttData').datagrid({url: `{{ url("$url/ajax?action=datagrid-jadwal-komite") }}&jadw_id={{$dataJadwal->jadw_id}}`});
				},
				...
				onEndEdit:function(index,row){
					...
					var itms_komite_posisi = $(this).datagrid('getEditor', {index: index,field: 'komite_posisi'});
					if(itms_komite_posisi !== null){
						row.komite_posisi = $(itms_komite_posisi.target).combobox('getValue');
					}
				},
                ...
                columns: [[
					...
					{field: 'komite_id', hidden: true},
					{field: 'peg_id', hidden: true},
                    {field: 'peg_nama', title: 'Nama', width: 300, sortable: true,
						editor:{
							type:'combogrid',
							options:{
								url:`{{ url("$url/ajax?action=combogrid-pegawai") }}&jadw_id={{$dataJadwal->jadw_id}}`,
								pageSize: '50',
								panelWidth: 650,
								pagination: true,
								idField: 'peg_id',
					...
					{field: 'komite_posisi', title: 'Posisi', width: 100, sortable: true,
						editor:{
							type:'combobox',
							options:{
								valueField:'id',
            ...
            dg.datagrid(
                'enableFilter', [
                    ...
                    {field: 'komite_posisi', type: 'textbox'},
                ]);
        });
		...
		function confirmSimpan() {
            ...
            swalWithBootstrapButtons({
                title: `Ajukan Data ?`,
                text: "Mengajukan Komite Audit akan memberikan informasi kepada pegawai yang ditunjuk sebagai komite untuk melakukan tugasnya?",
                type: 'warning',
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: `{{url("$url/update")}}`,
						data: { 'jadw_id': `{{$dataJadwal->jadw_id}}`, 'tipe': 'ajukan-komite' },
						type: 'POST',
                ...
                if (result.value) {
					...
					for (var i = 0; i < data.rows.length; i++) {
						...
						if(atLeastOneIsChecked == true){
							idData.push(data.rows[i].komite_id);
						}
					}
                    $.ajax({
                        url: `{{url("$url/delete")}}`,
						data: { 'ids[]': idData, 'tipe': 'data-komite' },
						type: 'DELETE',
                        success: function (response) {
                            toastCenter({
                                type: 'success',
