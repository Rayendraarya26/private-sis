...
@section('title', 'Master Kode NACE')
                ...
                <div class="dt-card">
                    <div class="dt-card__header">
                        <div class="dt-card__heading">
                            <h3 class="dt-card__title">Data Master Kode NACE</h3>
                        </div>
                    </div>
    ...
    <script>
        $(function () {
            let dg = $('#ttData').datagrid({
                ...
                frozenColumns: [[
                    {field: 'ck', checkbox: true, sortable: false},
                    {
                        ...
                        formatter: function (val, row) {
                            let btnEdit = `<a href="{{url("$url/edit")}}/${row.kode_nace_id}" class="btn btn-primary btn-xs btn-block">Edit</a>`;
							let output = "";
                ...
                columns: [[
                    {field: 'kode_nace_nama', title: 'Kode NACE', width: 400, sortable: true},
                ]],
            });
            dg.datagrid(
                'enableFilter', [
                    {field: 'action', type: 'label'},
                    {field: 'kode_nace_nama', type: 'textbox'},
                ]);
        });
                ...
                if (result.value) {
					...
					for (var i = 0; i < data.rows.length; i++) {
						...
						if(atLeastOneIsChecked == true){
							idData.push(data.rows[i].kode_nace_id);
						}
					}
                    $.ajax({
                        url: `{{url("$url/delete")}}`,
