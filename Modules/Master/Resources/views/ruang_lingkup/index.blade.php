                ...
                <div class="dt-card">
                    <div class="dt-card__header">
                        <div class="dt-card__heading">
                            <h3 class="dt-card__title">Data master Ruang Lingkup</h3>
                        </div>
                    </div>
    ...
    <script>
        $(function () {
            let dg = $('#ttData').datagrid({
                ...
                frozenColumns: [[
                    {field: 'ck', checkbox: true, sortable: false},
                    {
                        ...
                        formatter: function (val, row) {
                            let btnEdit = `<a href="{{url("$url/edit")}}/${row.ruang_ling_id}" class="btn btn-primary btn-xs btn-block">Edit</a>`;
                            let output = "";
                ...
                columns: [[
                    // {field: 'negara_kode', title: 'Kode Ruang Lingkup', width: 220, sortable: true},
                    {field: 'ruang_ling_nama', title: 'Ruang Lingkup', width: 300, sortable: true},
                ]],
            });
            dg.datagrid(
                'enableFilter', [
                    ...
                    // {field: 'negara_kode', type: 'textbox'},
                ]);
        });
                ...
                if (result.value) {
					...
					for (var i = 0; i < data.rows.length; i++) {
						...
						if(atLeastOneIsChecked == true){
							idData.push(data.rows[i].ruang_ling_id);
						}
					}
                    $.ajax({
                        url: `{{url("$url/delete")}}`,
