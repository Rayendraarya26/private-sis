                ...
                <div class="dt-card">
                    <div class="dt-card__header">
                        <div class="dt-card__heading">
                            <h3 class="dt-card__title">Data Master Kode EA</h3>
                        </div>
                    </div>
    ...
    <script>
        $(function () {
            let dg = $('#ttData').datagrid({
                ...
                frozenColumns: [[
                    {field: 'ck', checkbox: true, sortable: false},
                    {
                        ...
                        formatter: function (val, row) {
                            let btnEdit = `<a href="{{url("$url/edit")}}/${row.kode_ea_id}" class="btn btn-primary btn-xs btn-block">Edit</a>`;
							let output = "";
                ...
                columns: [[
                    {field: 'kode_ea_nama', title: 'Kode EA', width: 400, sortable: true},
                ]],
            });
            dg.datagrid(
                'enableFilter', [
                    {field: 'action', type: 'label'},
                    {field: 'kode_ea_nama', type: 'textbox'},
                ]);
        });
                ...
                if (result.value) {
					...
					for (var i = 0; i < data.rows.length; i++) {
						...
						if(atLeastOneIsChecked == true){
							idData.push(data.rows[i].kode_ea_id);
						}
					}
                    $.ajax({
                        url: `{{url("$url/delete")}}`,
