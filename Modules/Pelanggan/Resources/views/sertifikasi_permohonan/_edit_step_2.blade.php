    ...
    <script>
        $(document).ready(function () {
            window.vueStepTwo = new Vue({
                ...
                methods: {
                    start() {
                        @foreach($dataPemohon->sis_permohonan_komoditis as $komoditi)
                            this.komoditas.push({
                        ...
                        setTimeout(async () => {
                            ...
                            let currentData = {!! json_encode($dataPemohon->master_sertifikasi) !!};
                            $('#step2_jenis_sertifikasi').combogrid('setValue', currentData.sert_id)
                            this.comboDataSertifikasiOnSelect(currentData)
                    ...
                    uploadDokumen(id) {
                        ...
                        $.ajax({
                            url: `{!! url("$url/ajax?action=permohonan_unggah_dokumen&mohon_id=" . $dataPemohon->mohon_id) !!}`,
                            type: 'post',
                            processData: false,
                            contentType: false,
                            data: formData,
                    ...
                    async deleteKomoditi(idx) {
                        let selectedKomoditas = this.komoditas[idx]
                        swalWithBootstrapButtons({
                    ...
                    async editKomoditi(idx) {
                        ...
                        this.jenis_komoditas_form_edited_id = idx;
                    },
                    async updateKomoditi() {
                        try {
                            this.validateKomoditas()
                            this.komoditas[this.jenis_komoditas_form_edited_id] = {
                                "komoditi_id": this.jenis_komoditas_id,
                    ...
                    getDokumenSertifikasi() {
                        $.get(`{!! url("$url/ajax?action=permohonan_get_dokumen") . '&mohon_id=' . $dataPemohon->mohon_id !!}`)
                            .then(response => {
