...
use App\Models\BbkkpSis\PegawaiKompetensiAuditor;
...
class KompetensiAuditorController extends Controller
{
    ...
    private $url = 'admin/kompetensi/auditor';
    private $view = 'admin::kompetensi_auditor';

    public function index()
    {
        $breadcrumbs = [
            new BreadcrumbsStruct('Administrator'),
            new BreadcrumbsStruct('Kompetensi Auditor'),
        ];
    ...
    {
        try {
            $dataPegawai = MasterPegawai::with('pegawai_kompetensi_auditors')->find($pegawaiId);
            ...
            foreach ($dataPegawai->pegawai_kompetensi_auditors as $kompetensi_auditor) {
                $selectedSertifikatId[] = $kompetensi_auditor->sert_id;
            }
    ...
    public function saveByPegawai(Request $request)
    {
        try {
            ...
            if ($request['value'] == 'allow') {
                PegawaiKompetensiAuditor::create([
                ...
                return responseJSON(200, [], sprintf('Berhasil memberikan izin menjadi auditor pada "%s"', $sertifikat->sert_nama));
            } else {
                PegawaiKompetensiAuditor::where('peg_id', $request['peg_id'])->where('sert_id', $request['sert_id'])->delete();
                return responseJSON(200, [], sprintf('Izin auditor "%s" berhasil dicabut', $sertifikat->sert_nama));
            }
        } catch (Exception $e) {
    ...
    private function ajax_datagrid_by_pegawai(Request $request)
    {
        $data = SysUser::leftJoin('master_pegawai', 'master_pegawai.user_id', '=', 'sys_user.user_id')
            ...
            ->with('master_pegawai.pegawai_kompetensi_auditors.master_sertifikasi')
            ->whereNotIn('sys_user_group.ug_group_id', [1, 3])
            ->where('is_auditor', '=', 'yes');
        ...
        foreach ($data->get() as $d) {
            $dataKompetensi = [];
            foreach ($d->master_pegawai->pegawai_kompetensi_auditors as $kompetensi) {
