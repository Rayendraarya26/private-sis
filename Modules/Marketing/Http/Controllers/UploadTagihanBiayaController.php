...
class UploadTagihanBiayaController extends Controller
{
    ...
    private $url = 'marketing/tagihan-biaya';

    public function index()
    {
        $breadcrumbs = [
            new BreadcrumbsStruct('Marketing'),
            new BreadcrumbsStruct('Surat Tagihan Biaya'),
        ];

        $parser = ['module' => $this->module, 'url' => $this->url, 'breadcrumbs' => $breadcrumbs];
        return view("marketing::tagihan_biaya.index")->with($parser);
    }
    ...
    private function detail_permohonan(Request $request, $mohonID)
    {
        ...
        $breadcrumbs = [
            new BreadcrumbsStruct('Marketing'),
            new BreadcrumbsStruct('Surat Tagihan Biaya', url($this->url)),
    ...
    public function edit(Request $request) // menerima parameter ID dari Modules\Master\Routes\web.php
    {
        ...
        return match ($request['status']) { // Match fitur mirip switch case tetapi lebih simple (PHP 8 keatas)
            'upload-kajian-permohonan' => $this->edit_upload_tagihan_biaya($request),
            default                    => null,
        };
    }

    private function edit_upload_tagihan_biaya(Request $request)
    {
        $breadcrumbs = [
            new BreadcrumbsStruct('Marketing'),
            new BreadcrumbsStruct('Surat Tagihan Biaya', url($this->url)),
