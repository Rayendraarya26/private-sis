...
class AuLogBookController extends Controller
{
    ...
    private $url = 'timaudit/auditor/log-book';
		...
		$data->leftJoin('sis_audit_logbook', function($join)
                         {
                             ...
                             $join->on('logbook_jenis','=',DB::raw("'auditor'"));
                         });
	...
	private function update_upload_logbook(Request $request)
	{
		...
		try {
            ...
            $fileJadwalName = Str::slug('file-logbook-auditor-'. $request['jadw_tim_id'] .'-'. $fileJadwal->getClientOriginalName()) . '-' . time() . '.' . $fileJadwal->getClientOriginalExtension();
