...
class KomiteLembarPeriksaController extends Controller
{
    ...
    private function ajax_datagrid_jadwal_audit(Request $request)
    {
        ...
        $data->leftJoin('sis_audit_komite_periksa', "sis_audit_komite_periksa.jadw_id", "=", "sis_jadwal.jadw_id");
        ...
        foreach ($data->get() as $d) {
            ...
            $x['komte_priksa_id']      = ($d->komte_priksa_id != '') ? $d->komte_priksa_id : '';
            $result[]                  = $x;
        }
    ...
    public function edit(Request $request)
    {
        ...
        return match ($request['tipe']) {
            ...
            'lihat-lembar-periksa' => $this->edit_lihat_lembar_periksa($request),
            default                => null,
        };
    }

    private function edit_lihat_lembar_periksa(Request $request)
    {
    ...
    private function update_lembar_periksa(Request $request)
    {
        $request->validate([
            ...
            "status"                          => 'nullable',
            'jadw_file_kehadiran_komite'      => 'nullable',
            'posisi'                          => 'nullable',
        ...
        try {
            if (str_contains($request['posisi'], 'ketua') && (!$request->hasFile('jadw_file_kehadiran_komite'))) {
                throw new ExpectedException("Mohon unggah file jadwal", 400);
            }

            DB::beginTransaction();

            $restJadwal = SisJadwal::select('*')->where('jadw_id', $request['jadw_id'])->first();
            if (empty($restJadwal)) throw new ExpectedException('Data jadwal tidak ditemukan');

            if (str_contains($request['posisi'], 'ketua')) {
